...
- 建立受 DB 約束保護的 `原生家庭` system context tag，保守推定孝親費與具親屬＋支出語意的項目，並在修改品項時重新計算。
...
- 實作持久的群組個人／共同模式、狀態查詢與明確 scope 單筆覆蓋。
- 無篩選最近／期間／排行依 LINE user 跨聊天視窗顯示操作者的完整個人資料；共同與全部使用明確篩選，`本月 全部`回傳操作者個人小計與目前配對共同小計，不含對方個人帳。
